...
在原有四大业务模块基础上，新增企业控制中心：支持组织与账期维度、幂等防重、经办/管理员职责分离、审批闭环、附件 SHA-256 元数据、办结凭证门槛、外部适配器回执、乐观锁和全程审计；并实现多主体本位币折算、试算平衡、抵销平衡与关账阻断。详见[企业版能力说明](docs/ENTERPRISE.md)。
...
新增正式关账前的企业认证门禁，统一核对子账、银行对账、内部往来、试算平衡、抵销、税务复核、重大调整、职责分离和管理层签署。详见[企业关账认证说明](docs/ENTERPRISE_CLOSE_CERTIFICATION.md)。
